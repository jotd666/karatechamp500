...
indir = "mame_gfxsave"
palette_name = "palette0 256_0000.txt"
game_name = "kchampvs2"
groups = {0:{"name":"tile"},1:{"name":"sprite"},2:{"name":"sprite"},3:{"name":"sprite"}}
text_bitmap = " .=#"
def get_clut(tile_group_index,clut_index):
    if tile_group_index == 0:
        clut_index+=32
...
with open(os.path.join(indir,palette_name),encoding='utf-8-sig') as f:
    line = next(f).split()
...
#define NUM_COLOURS {len(palette)}
...
"""
    ...
    """)
        ...
        blocks = itertools.chain.from_iterable(groups[i]["data"] for i in range(1,4))
        write_tiles(blocks,size,f)
